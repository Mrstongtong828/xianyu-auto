...
- **自动收集** - 消息触发时自动收集商品信息
- **商品详情** - 支持获取、查看和编辑商品详情
- **发布后可见** - 授权账号发布成功并拿到商品 ID 后，会立即写入本地商品缓存，商品管理页无需等待完整详情同步即可看到新商品
- **多规格配置** - 支持多规格商品配置和管理
- **智能去重** - 自动去重，避免重复存储
...
# 授权账号单品发布（可选）
XIANYU_PUBLISH_URL=https://www.goofish.com/publish
XIANYU_PUBLISH_SELECTOR_CONFIG=config/item_publish_selectors.json
```

> 其他运行参数（如 WebSocket、心跳、自动回复等）主要在 `global_config.yml` 和 Web 管理界面中配置。
> 授权账号单品发布功能需要可见浏览器或 VNC 环境，选择器维护说明见 `docs/item-publish-guide.md`。

### 📝 本次更新

- 修复授权账号发布商品成功后，商品管理页按账号筛选时看不到新商品的问题。
- 发布接口拿到 `published_item_id` 后，会立即向 `item_info` 写入一条最小商品记录，包含发布账号、商品 ID、标题、价格和描述等可用信息。
- 发布后的同步结果现在以本地是否能查到目标商品为准，避免 API 拉取执行过但未确认目标商品时误报为完全同步成功。
- 商品缓存保存逻辑保持幂等：已存在记录只补空字段，不覆盖后续同步得到的完整详情。
- 前端发布成功后会刷新发布日志和商品管理本地数据；如果用户仍在商品发布页，不会被强制跳转。
- 新增单元测试覆盖发布后最小记录、同步确认语义、详情补全以及 `/items`、`/items/cookie/{cookie_id}` 返回新商品的场景。
